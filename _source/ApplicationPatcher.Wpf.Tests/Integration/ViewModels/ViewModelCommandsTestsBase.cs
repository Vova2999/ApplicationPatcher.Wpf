...
namespace ApplicationPatcher.Wpf.Tests.Integration.ViewModels {
	public abstract class ViewModelCommandsTestsBase {
		protected static bool CanExecuteCommand<TViewModel>(TViewModel viewModel, string propertyName) where TViewModel : ViewModelBase {
		...
		protected static void ExecuteCommand<TViewModel>(TViewModel viewModel, string propertyName) where TViewModel : ViewModelBase {
			var propertyInfo = typeof(TViewModel).GetProperty(propertyName);
			propertyInfo.Should().NotBeNull();
